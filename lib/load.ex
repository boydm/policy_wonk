defmodule PolicyWonk.Load do
  ...
  In a plug stack, code is run before your controller’s actions, so you need to use `PolicyWonk.Load` (or equivalent) to load resources into the conn’s `assigns` field before running the `PolicyWonk.Enforce` plug.

  In a controller…

      plug PolicyWonk.Load, :thing_a
  ...
  The result is that your `:current_user` loader function is called. If it succeeds, it returns a resource (assumedly the current user…), which `PolicyWonk.Load` adds to the conn’s `assigns` field

  Please see documentation for `PolicyWonk.Resource` to see how to implement your loaders.

  ## Specifying Loaders

  The main parameter to the `PolicyWonk.Load` plug is either a single resource or a list of resources to load.

        plug PolicyWonk.Load, :thing_a
        plug PolicyWonk.Load, [:thing_a, :thing_b]
  ...
  These are all valid resource specifiers:
        ...
        plug PolicyWonk.Load, [:thing_a, :thing_b]
        plug PolicyWonk.Load, {:thing_s, "a string")
        plug PolicyWonk.Load, %{id: "an_id", data: %{color: "blue"}}
  ...
  * `resource` this is the loaded resource itself
  ...
  In other words, if you return the tuple `{:ok, :name,"policy_wonk"}`, then when `PolicyWonk.Load` is finished doing it’s work, `conn.assigns.name` will be `"policy_wonk"`.
  ...
  `PolicyWonk.Load` helps by (optionally) loading the resources you specify in any given call asynchronously.
  ...
  You can also request asynchronous loading with the expanded form of the plug invocation.
      ...
      plug PolicyWonk.Load, %{resources: [:thing_a, :thing_b], async: true}

  ## Use with Guards

  When the `PolicyWonk.Load` is invoked inside a Phoenix controller, you can add guards against the current action.

      plug PolicyWonk.Load, :thing_a when action in [:index]


  ## Handling Load Failures

  If any call to a `load_resource` function fails, then the `PolicyWonk.Load` plug calls your `load_error` function with the data returned by the loader.
  ...
  As discussed in [the documentation for PolicyWonk.Resource](PolicyWonk.Resource.html#module-loader-locations), 
  the `PolicyWonk.Load` plug will look for loaders first in your controller (or router) as appropriate. Then in the module/s specified in the config block.
  ...
  You can also specify exactly which module to look in at the time you invoke the plug.
      ...
      plug PolicyWonk.Load, %{resources: [:thing_1], module: MyLoaderModule}
  ...
  [See the discussion of specifying loaders above.](PolicyWonk.Load.html#module-specifying-loaders)
  """
